const SCRIPT_URL_DATABASE = "https://script.google.com/macros/s/AKfycbzgdZ94RLrLiG6PTd4IVvVAx3d4KsPNmLwqzViPedkY-Houk6QWEcgFweEcQgW2IxfkEg/exec";
...
function buildContinuationRow(remainingHTML, padLevel) {
    ...
    tr.innerHTML = `
        <td></td>
        <td class="uraian-cell" style="font-style: italic; color: #94a3b8; font-size: 9px; padding-top: 4px;"></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
...
function cetakPro() {
    ...
    Swal.fire({ title: 'Sedang Memproses...', text: 'Sistem Sedang Menyusun Halaman...', allowOutsideClick: false, didOpen: () => { Swal.showLoading(); }});
...
function simpanKeCloud() {
    ...
    let userAgent = navigator.userAgent.substring(0, 150);
    let userName = localStorage.getItem('LRA_USER_NAME') || '';
    let identity = userName ? `${userName} | ${userAgent}` : userAgent;
