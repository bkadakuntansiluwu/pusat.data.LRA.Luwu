...
function cetakPro() {
    ...
    setTimeout(() => {
        ...
        rows.forEach(row => {
            ...
            // SENSOR PRESISI TINGGI: Ukur per baris, bukan seluruh tabel
            let rowRect = clone.getBoundingClientRect();
            let footerRect = currentFooter.getBoundingClientRect();

            // SENSOR SUPER MEPET (Toleransi hanya 5 pixel)
            if (rowRect.bottom > (footerRect.top - 5)) {
        ...
        // Sensor tanda tangan juga dibuat mepet (toleransi 5 pixel)
    ...
    fetch(fetchUrl)
        .then(r => r.json()).then(res => {
            if(res.status === 'success') {
                ...
                text: 'Belum ada penjelasan yang tersimpan untuk SKPD dan tahun ini.',
                confirmButtonText: 'OK'
            });
        }).catch(() => Swal.fire({
            icon: 'error',
            title: 'Koneksi Terputus',
            text: 'Gagal terhubung ke server. Periksa koneksi internet Anda, lalu coba Tarik Data kembali.',
            confirmButtonText: 'OK'
        }));
}
